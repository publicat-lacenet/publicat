...
Aquest document defineix la lògica reactiva de la base de dades. Els triggers garanteixen la integritat de les dades, automatitzen tasques repetitives i gestionen les notificacions del sistema de moderació.
...
Quan es crea un centre, es generen automàticament les 8 llistes predefinides.
...
- [ ] Trigger `set_updated_at` funciona en totes les taules.
- [ ] Sincronització d'email des de `auth.users` verificada.
- [ ] La creació d'un centre genera les 8 llistes automàticament.
- [ ] Les notificacions es generen per a tots els `editor_profe` del centre.
- [ ] Els triggers s'executen amb `SECURITY DEFINER` per evitar problemes de RLS.

---

## 6. Pròxims Passos
- Executar seeds de prova a `M1-6-seeds.md`.
