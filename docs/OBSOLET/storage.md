...
> Nota 2026-07-09: aquest document necessita alineació amb l'estat real. La verificació actual només ha detectat el bucket públic `announcement-frames` i cap policy visible a `storage.objects`; les seccions sobre `center-logos` i `video-thumbnails` s'han de tractar com a disseny/històric fins que es revisin.
